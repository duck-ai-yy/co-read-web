...
> Architect subagent · 2026-05-19
> 状态：**design only · 待鸭鸭审 · 未动代码**
...
v3 把 co-read 从"一篇 PDF 的伴读"升级成"**研究流容器**"：顶层引入**主题（Topic）**——一组论文 + 一套创建时锁定的色板 + 一份导出笔记。色板冻结让一个主题内所有论文共享同一个 system prefix → **跨论文 caching 命中**；同时 agent 输出强制带 `〔p.N〕` 回链，前端 marked.js 渲染成可点链接，点击 = 跳页 + 闪烁定位。**主要风险**：数据迁移（v2-b 已有 IndexedDB annotations）+ system_prompt.md 要不要拆（颜色定义从 prompt 移到注入）——这两件请鸭鸭拍板。
...
> 完全兼容现 6 色语义。鸭鸭在新建主题时可改 emoji / label / 增删 / 改 promptHint。
...
**这是改 system_prompt.md 的事，鸭鸭决定要不要改**（见 G5）。
...
| B. 手动选 / 重建 | 第一次打开 v3 弹个迁移向导，让用户给每个 PDF 选主题 | 干净 | 鸭鸭一个人用的 alpha 阶段没必要弹向导 |
| C. 兼容模式 | 老数据无 topicId，列表里单独显示"未归类"区 | 渐进 | 数据模型变成两套（有/无 topicId），代码复杂度上升 |

**推荐 A**。`co-read` 还是鸭鸭个人 alpha，简单稳。Migration 函数大致：
...
### G1. 数据迁移路径（A/B/C）
推荐 **A 自动归入"默认主题"**。理由：alpha 阶段、单用户、老数据规模小、无感知最优。**鸭鸭你点头我就这么做**。
...
推荐 **MVP 不允许，固定 6 个默认 emoji**（红绿蓝紫黄灰）。emoji-mart picker 30KB+ 不划算。**让用户改的是 label / color / promptHint / 增删行**。鸭鸭如果觉得这 6 个 emoji 够用，就这么定。
...
### G5. system_prompt.md 鸭鸭要不要改 ★ 最关键
...
**架构师建议走 Y**（解耦设计原则、配置驱动）。但这是改 system_prompt.md，**严格按 architect.md 的纪律：subagent 不动 system_prompt.md，必须鸭鸭决定**。

> 鸭鸭：v3 上线时你需要决定走 X / Y / Z。如果选 Y，建议你亲自改 system_prompt.md（删 L16-22 的 6 色定义，留一段"按色板规则回应"的元规则），不让 subagent 代笔。
...
鸭鸭打开 v3 第一次（迁移完成后），是看到 1 个"默认主题"卡片 + 提示「点这里继续」，还是直接进入唯一主题的主题页？**推荐前者**——3 层导航的认知一致性优先于"少一次点击"。

### G8. v2-b 单击 hl-rect 的 UX 决策（v2-b 报告留的尾巴）
v2-b 测试报告 P49-53 留了个问题："单击 = 仅浮气泡" 还是 "单击 = 切 thread"。v3 不解决这个，**继续沿用 v2-b 现在的"仅浮气泡"行为**。鸭鸭如果在 v3 期间使用感觉不爽，再单独提 issue。
...
> 还没到 test 阶段，先草拟一个，鸭鸭审 doc 时可以挑刺。
...
| 鸭鸭对走向 Y 不放心 → 三套规则共存（X） | 中 | 中 | 实现时先按 Z（默认色板对齐）做，Y 改 prompt 等鸭鸭拍板 |
...
**待鸭鸭确认 G1~G8 → 主 Claude 拆任务给 dev → test 按 I 节验收**。
